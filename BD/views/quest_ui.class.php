...
protected $Error;
	...
	public function DisplayNextActivityQuests()
	{
		...
		$lActivity = $_SESSION['masterlist']->GetNextMainActivity();
			if($lActivity === False) { echo "Il n'y a pas de prochaine activité définie pour le moment. Merci de patienter! =)"; return;}
		$lActivityName = $lActivity->GetName();
			if( $lActivityName == "" ) { $lActivityName = "GN indéterminé"; }
		$lPersonalQuestList = $this->Manager->GetPersonalQuests();
		$lGroupQuestList = $this->Manager->GetGroupQuests();

		// Display the title.
		echo '<div>';
		echo '<span class="section-title">Quêtes approuvées pour '.$lActivityName.'</span>';
		echo '<hr width=70% />';

		echo '<form method="post">';
		echo '<input type="hidden" name="action" value="manage-quest-lists"/>';
		echo '<input type="hidden" name="assignee" value="'.$_SESSION['player']->GetID().'"/>';
		...
		echo '<tr>
			...
			<th class="black-cell" style="width:100px;">Comté choisi</th>
			<th class="black-cell" style="width:180px;">Scripteur</th>
			<th class="black-cell" style="width:80px;">État</th>
			<th class="black-cell" style="width:30px;"></th>
		      </tr>';


		foreach( $lPersonalQuestList as $i => $quest ) {
			if( $quest->GetActivity()->GetID() == $lActivity->GetID() ) {
				// Gather quest data
				...
				$lButton =	'<button type="submit" name="select-personal-quest" value="' .$quest->GetID(). '" class="icon-button"/><img src="images/icon_see.png" title="Consulter la quête" class="icon-button-image"></button>
						 <button type="submit" name="assign-personal-quest" value="' .$quest->GetID(). '" class="icon-button"/><img src="images/icon_assign.png" title="M\'assigner la quête" class="icon-button-image"></button>';

				$lCountyName = $quest->GetCountyName();
					if( $lCountyName == "" ) { $lCountyName = "Aucun"; }

				$lScriptorInput = '<select name="scriptor"><option value="0"></option>';
				foreach($this->Manager->GetScriptors() as $scriptor) {
					$selected = ''; if( $quest->GetScriptor() && $quest->GetScriptor()->GetID() == $scriptor->GetID() ) { $selected = 'selected'; }
					$lScriptorInput .= '<option value="'. $scriptor->GetID() .'" '.$selected.'>'. $scriptor->GetFullName() .'</option>';
				}
				$lScriptorInput .= '</select>';
				$lScriptorName = $quest->GetScriptor()->GetFullName();

				// Display quest entry
				echo '<tr>';
				echo '<td class="grey-cell" style="width:20px;">'	.$quest->GetID().	'</td>'; 
				echo '<td class="white-cell" style="width:150px;">'	.$lCharacterName.	'</td>';
				echo '<td class="white-cell" style="width:150px;">'	.$quest->GetSubject().	'</td>';
				echo '<td class="white-cell" style="width:100px;">'	.$lCountyName.		'</td>';
				echo '<td class="white-cell" style="width:180px;">'	.$lScriptorName.	'</td>';
	...
	//--DISPLAY MY ASSIGNED QUESTS--
	public function DisplayMyQuests()
	{
		// Check if there's a quest manager...
		if( $this->Manager == null ) { $this->Error = "No quest manager defined!"; return; }
		...
		// Display the title.
		echo '<div>';
		echo '<span class="section-title">Quêtes personnelles</span>';
		echo '<hr width=70% />';
		...
		foreach( $lPersonalQuestList as $i => $quest ) {
			...
			elseif( $quest->GetScriptor()->GetID() == $_SESSION['player']->GetID() ){
				...
				$lCountyName = $quest->GetCountyName();
					if( $lCountyName == "" ) { $lCountyName = "Aucun"; }
 				$lActivityName = $quest->GetActivity()->GetName();
					if( $lActivityName == "" ) { $lActivityName = "Indéterminée"; }
				...
				echo '<td class="white-cell" style="width:100px;">'	.$lActivityName.	'</td>';
	...
	public function DisplayUnassignedQuests()
	{
		// Check if there's a quest manager...
		if( $this->Manager == null ) { $this->Error = "No quest manager defined!"; return; }
		...
		// Display the title.
		echo '<div>';
		echo '<span class="section-title">Quêtes personnelles à assigner</span>';
		echo '<hr width=70% />';
...
//=================================================================== MANAGEMENT UIs ===================================================================
	...
	public function DisplayQuestLists($inActivityList = array(), $inStatusList = array(), $inUserIDList = array(), $inCharacterIDList = array(), $inSkillCodeList = array())
	{
		// Check if there's a quest manager...
		if( $this->Manager == null ) { $this->Error = "No quest manager defined!"; return; }
