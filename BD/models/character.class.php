...
define('SYLLABUS_UPLOAD_DIR', 'uploads/Plans de cours/');
define('LETTERS_UPLOAD_DIR', 'uploads/Missives/');
...
protected $AnsweredSurveys;		
	...
	public function __construct( $inDataArray =array() )
	{
		...
		if( isset($inDataArray['syllabuses']) ) 	{ $this->Syllabuses = $inDataArray['syllabuses']; }
	...
	public function GetQuestCredits() { return $this->QuestCredits; }
	...
	public function GetSyllabuses() { return $this->Syllabuses; }
	...
	public function SetSkills($inList) { $this->Skills = $inList; }
	public function SetTalents($inList) { $this->Talents = $inList; }
	public function SetTeachings($inList) { $this->Teachings = $inList; }
	public function SetSyllabuses($inList) { $this->Syllabuses = $inList; }
	...
	public function SetApprovals($inList) { $this->Approvals = $inList; }
	...
	public function GetTotalMana() {
		...
		foreach ($this->Talents as $talent) {
			    if( $talent['code'] == 'RESERVE' && $talent['status'] != 'INACT') { $lMana += 20; }
		}
		return $lMana; 
	}
	...
	public function GetFullName() { 
		$lFullName = $this->FirstName . ' ' . $this->LastName; 
		if(WORLD == 'TERNOC') { $lFullName .= ' - ID#'.$this->ID; }
		return $lFullName; }
	...
	public function GetQuestCreditsByType( $inType ='UNIV' ) {
		if( !isset($this->QuestCredits[$inType]) ) { return 0; }

		return $this->QuestCredits[$inType]; 
	}

	public function GetSubjectApproval( $inSubject ) {
		foreach ($this->Approvals as $approval) {
		 	if( $approval->GetSubject() == $inSubject ) { return $approval; }
		}
		return False;
	}
	...
	public function GetNextInvitation() {
		$lNextInvitation = null;
		if( count($this->Invitations) ) { $lNextInvitation = $this->Invitations[0]; }
		return $lNextInvitation;
	}
