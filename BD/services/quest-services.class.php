...
/*
╔══CLASS════════════════════════════════════════════════════════╗
║	== Quest Management Services v1.2 r12 ==		║
...
public $Error;
	...
	public function GetPersonalQuestLists()
	{
		...
		// Ask the database...
		$lQuery = 	"SELECT que.Id, que.IdPersonnage, per.Prenom AS PrenomPerso, per.Nom AS NomPerso, 
		...
		foreach ($r as $quest) {
			...
			if ($attributes['optioncode'] == 'MYTHIQ') { $lMythicQuestList[] = new Quest( $attributes ); }
			else { $lQuestList[] = new Quest( $attributes ); }

		}

		$this->Manager->SetPersonalQuests( $lQuestList );
		$this->Manager->SetMythicQuests( $lMythicQuestList );

		return True;
	}


	//--GET ALL GROUP QUESTS--
	public function GetGroupQuestLists()
	{
		...
		// Ask the database...
		$lQuery = 	"SELECT que.Id, que.IdGroupe, grp.Nom AS NomGroupe, 
					que.Objet, que.CodeOption, que.CodeRecompense, que.Suggestions, que.Fichier, que.CodeEtat, que.DateDemande, que.DateApprobation, que.Commentaires,
					que.IdComte, cmt.Nom AS NomComte, duc.CodeRoyaume, roy.Nom AS NomRoyaume,
					que.IdActivite, act.Nom AS NomActivite, act.DateDebut,
					que.IdResponsable, rev.Prenom AS PrenomResponsable, rev.Nom AS NomResponsable, 
					que.IdScripteur, scr.Prenom AS PrenomScripteur, scr.Nom AS NomScripteur, 
					que.IdDemandeur, dem.Prenom AS PrenomJoueur, dem.Nom AS NomJoueur, dem.Courriel
	...
	public function GetRevisorList()
	{
		...
		foreach ($r as $revisor) {
			$attributes = [
					'id'		=>	$revisor['Id'],
					'firstname'	=>	$revisor['Prenom'],
					'lastname'	=>	$revisor['Nom']
			];

			$lRevisorList[] = new User( $attributes );
		}

		$this->Manager->SetRevisors( $lRevisorList );
	...
	public function UpdateQuestRevisor( $inRevisorID )
	{
		// Verify there's a manager
		if( !isset($this->Manager) ) { $this->Error = "No quest manager!"; return False; }
		...
		if( $this->Manager->GetSelectedQuest()->GetType() == 'PERSONAL' ) {
			$lQuery = "UPDATE db_perso.quetes que
				   SET que.IdResponsable = :revisorid
				   WHERE que.Id = :questid;";

			$this->DAL->SetQuery($lQuery);
				$this->DAL->Bind(":questid", $this->Manager->GetSelectedQuest()->GetID(), PDO::PARAM_INT);
				$this->DAL->Bind(":revisorid", $inRevisorID, PDO::PARAM_INT);
			$r = $this->DAL->FetchResult();
	...
	public function UpdateQuestScriptor( $inScriptorID )
	{
		...
		if( $this->Manager->GetSelectedQuest()->GetType() == 'PERSONAL' ) {
			$lQuery = "UPDATE db_perso.quetes que
				   SET que.IdScripteur = :scriptorid
				   WHERE que.Id = :questid;";
	...
	//--UPDATE EXISTING QUEST'S SCRIPTOR--
	public function UpdateQuestComments( $inComments )
	{
		// Verify there's a manager
		if( !isset($this->Manager) ) { $this->Error = "No quest manager!"; return False; }
	...
	//--GIVE QUEST REWARD--
	public function GivePersonalQuestReward( $inUIInput =0 )
	{
		// Verify there's a manager
		if( !isset($this->Manager) ) { $this->Error = "No quest manager!"; return False; }
		...
		// Determine case
		    if( $lOption == 'GAINXP' ) 	{ $this->GiveXPReward( $inUIInput ); }
